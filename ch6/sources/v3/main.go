...
func bodyCheck(id string) int {
	...
	print("\tgoroutine-", id, "-idCheck: bodyCheck ok\n")
	return bodyCheckTmCost
}

func xRayCheck(id string) int {
	time.Sleep(time.Millisecond * time.Duration(xRayCheckTmCost))
	print("\tgoroutine-", id, "-idCheck: xRayCheck ok\n")
	return xRayCheckTmCost
}

func start(id string, f func(string) int,
	next chan<- struct{}) (chan<- struct{}, chan<- struct{}, <-chan int) {
...
func main() {
	...
	AirportSecurityCheckChannel("channel1", queue)
	AirportSecurityCheckChannel("channel2", queue)
	AirportSecurityCheckChannel("channel3", queue)
